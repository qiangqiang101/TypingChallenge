...
Public Class BaseControl
    ...
    Public Sub PaintKeyboard(g As Graphics, offset As Point)
        ...
        Dim ks = 60, lks = 131, bks = 70, tks = 100, sks = ks * 7, eks = 141, ls = 15
        Dim xy = 10
        ...
        Dim numlight As New Rectangle(xy + pause.X + pause.Width + 50, rect.Y + (xy * 3), ls, ls)
        Dim caplight As New Rectangle(xy + numlight.X + numlight.Width + 50, rect.Y + (xy * 3), ls, ls)
        Dim scrlight As New Rectangle(xy + caplight.X + caplight.Width + 50, rect.Y + (xy * 3), ls, ls)
        ...
        If RGB Then
            ...
            Using lbrush As New LinearGradientBrush(rgbRect, Color.Red, Color.Blue, rgbAngle)
                ...
                Using pen As New Pen(lbrush, 3.0F)
                    g.DrawRoundedRectangle(keyboardRect, 10, pen)

                    Using kbFont As New Font("Segoe UI", 15.0F, FontStyle.Bold, GraphicsUnit.Pixel)
                        DrawRGBKeys(g, esc, pen, lbrush, "ESC", kbFont, Keys.Escape)
                        DrawRGBKeys(g, f1, pen, lbrush, "F1", kbFont, Keys.F1)
                        DrawRGBKeys(g, f2, pen, lbrush, "F2", kbFont, Keys.F2)
                        DrawRGBKeys(g, f3, pen, lbrush, "F3", kbFont, Keys.F3)
                        DrawRGBKeys(g, f4, pen, lbrush, "F4", kbFont, Keys.F4)
                        DrawRGBKeys(g, f5, pen, lbrush, "F5", kbFont, Keys.F5)
                        DrawRGBKeys(g, f6, pen, lbrush, "F6", kbFont, Keys.F6)
                        DrawRGBKeys(g, f7, pen, lbrush, "F7", kbFont, Keys.F7)
                        DrawRGBKeys(g, f8, pen, lbrush, "F8", kbFont, Keys.F8)
                        DrawRGBKeys(g, f9, pen, lbrush, "F9", kbFont, Keys.F9)
                        DrawRGBKeys(g, f10, pen, lbrush, "F10", kbFont, Keys.F10)
                        DrawRGBKeys(g, f11, pen, lbrush, "F11", kbFont, Keys.F11)
                        DrawRGBKeys(g, f12, pen, lbrush, "F12", kbFont, Keys.F12)
                        DrawRGBKeys(g, prnscn, pen, lbrush, $"Print{vbNewLine}Screen", kbFont, Keys.PrintScreen, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, scroll, pen, lbrush, $"Scroll{vbNewLine}Lock", kbFont, Keys.Scroll, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, pause, pen, lbrush, $"Pause{vbNewLine}Break", kbFont, Keys.Pause, TextFormatFlags.HorizontalCenter)
                        DrawRGBLights(g, numlight, pen, lbrush, My.Computer.Keyboard.NumLock)
                        DrawRGBLights(g, caplight, pen, lbrush, My.Computer.Keyboard.CapsLock)
                        DrawRGBLights(g, scrlight, pen, lbrush, My.Computer.Keyboard.ScrollLock)

                        DrawRGBKeys(g, tilde, pen, lbrush, $"~{vbNewLine}`", kbFont, Keys.Oemtilde)
                        DrawRGBKeys(g, d1, pen, lbrush, $"!{vbNewLine}1", kbFont, Keys.D1)
                        DrawRGBKeys(g, d2, pen, lbrush, $"@{vbNewLine}2", kbFont, Keys.D2)
                        DrawRGBKeys(g, d3, pen, lbrush, $"#{vbNewLine}3", kbFont, Keys.D3)
                        DrawRGBKeys(g, d4, pen, lbrush, $"${vbNewLine}4", kbFont, Keys.D4)
                        DrawRGBKeys(g, d5, pen, lbrush, $"%{vbNewLine}5", kbFont, Keys.D5)
                        DrawRGBKeys(g, d6, pen, lbrush, $"^{vbNewLine}6", kbFont, Keys.D6)
                        DrawRGBKeys(g, d7, pen, lbrush, $"＆{vbNewLine}7", kbFont, Keys.D7)
                        DrawRGBKeys(g, d8, pen, lbrush, $"*{vbNewLine}8", kbFont, Keys.D8)
                        DrawRGBKeys(g, d9, pen, lbrush, $"({vbNewLine}9", kbFont, Keys.D9)
                        DrawRGBKeys(g, d0, pen, lbrush, $"){vbNewLine}0", kbFont, Keys.D0)
                        DrawRGBKeys(g, minus, pen, lbrush, $"_{vbNewLine}-", kbFont, Keys.OemMinus)
                        DrawRGBKeys(g, plus, pen, lbrush, $"+{vbNewLine}=", kbFont, Keys.Oemplus)
                        DrawRGBKeys(g, bspace, pen, lbrush, "Backspace", kbFont, Keys.Back)
                        DrawRGBKeys(g, ins, pen, lbrush, "Insert", kbFont, Keys.Insert, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, home, pen, lbrush, "Home", kbFont, Keys.Home, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, pgup, pen, lbrush, $"Page{vbNewLine}Up{vbNewLine}↑", kbFont, Keys.PageUp, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, numlock, pen, lbrush, $"Num{vbNewLine}Lock", kbFont, Keys.NumLock, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, numslash, pen, lbrush, "/", kbFont, Keys.Divide)
                        DrawRGBKeys(g, numstar, pen, lbrush, "*", kbFont, Keys.Multiply)
                        DrawRGBKeys(g, numminus, pen, lbrush, "-", kbFont, Keys.Subtract)

                        DrawRGBKeys(g, tab, pen, lbrush, $"Tab{vbNewLine}⭾", kbFont, Keys.Tab)
                        DrawRGBKeys(g, q, pen, lbrush, "Q", kbFont, Keys.Q)
                        DrawRGBKeys(g, w, pen, lbrush, "W", kbFont, Keys.W)
                        DrawRGBKeys(g, e, pen, lbrush, "E", kbFont, Keys.E)
                        DrawRGBKeys(g, r, pen, lbrush, "R", kbFont, Keys.R)
                        DrawRGBKeys(g, t, pen, lbrush, "T", kbFont, Keys.T)
                        DrawRGBKeys(g, yy, pen, lbrush, "Y", kbFont, Keys.Y)
                        DrawRGBKeys(g, u, pen, lbrush, "U", kbFont, Keys.U)
                        DrawRGBKeys(g, i, pen, lbrush, "I", kbFont, Keys.I)
                        DrawRGBKeys(g, o, pen, lbrush, "O", kbFont, Keys.O)
                        DrawRGBKeys(g, p, pen, lbrush, "P", kbFont, Keys.P)
                        DrawRGBKeys(g, lbracket, pen, lbrush, "{" & vbNewLine & "[", kbFont, Keys.Oem4)
                        DrawRGBKeys(g, rbracket, pen, lbrush, "}" & vbNewLine & "]", kbFont, Keys.Oem6)
                        DrawRGBKeys(g, bslash, pen, lbrush, $"|{vbNewLine}\", kbFont, Keys.Oem5)
                        DrawRGBKeys(g, del, pen, lbrush, "Delete", kbFont, Keys.Delete, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, [end], pen, lbrush, "End", kbFont, Keys.End, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, pgdown, pen, lbrush, $"Page{vbNewLine}Down{vbNewLine}↓", kbFont, Keys.PageDown, TextFormatFlags.HorizontalCenter)
                        DrawRGBKeys(g, num7, pen, lbrush, $"7{vbNewLine}Home", kbFont, Keys.NumPad7)
                        DrawRGBKeys(g, num8, pen, lbrush, $"8{vbNewLine}↑", kbFont, Keys.NumPad8)
                        DrawRGBKeys(g, num9, pen, lbrush, $"9{vbNewLine}Pg Up", kbFont, Keys.NumPad9)
                        DrawRGBKeys(g, numplus, pen, lbrush, "+", kbFont, Keys.Add)

                        DrawRGBKeys(g, capslock, pen, lbrush, $"Caps Lock{vbNewLine}Ⓐ", kbFont, Keys.CapsLock)
                        DrawRGBKeys(g, a, pen, lbrush, "A", kbFont, Keys.A)
                        DrawRGBKeys(g, s, pen, lbrush, "S", kbFont, Keys.S)
                        DrawRGBKeys(g, d, pen, lbrush, "D", kbFont, Keys.D)
                        DrawRGBKeys(g, f, pen, lbrush, "F", kbFont, Keys.F)
                        DrawRGBKeys(g, gg, pen, lbrush, "G", kbFont, Keys.G)
                        DrawRGBKeys(g, h, pen, lbrush, "H", kbFont, Keys.H)
                        DrawRGBKeys(g, j, pen, lbrush, "J", kbFont, Keys.J)
                        DrawRGBKeys(g, k, pen, lbrush, "K", kbFont, Keys.K)
                        DrawRGBKeys(g, l, pen, lbrush, "L", kbFont, Keys.L)
                        DrawRGBKeys(g, semicolon, pen, lbrush, $":{vbNewLine};", kbFont, Keys.OemSemicolon)
                        DrawRGBKeys(g, quote, pen, lbrush, $"""{vbNewLine}'", kbFont, Keys.OemQuotes)
                        DrawRGBKeys(g, enter, pen, lbrush, $"Enter{vbNewLine}↵", kbFont, Keys.Enter)
                        DrawRGBKeys(g, num4, pen, lbrush, $"4{vbNewLine}←", kbFont, Keys.NumPad4)
                        DrawRGBKeys(g, num5, pen, lbrush, "5", kbFont, Keys.NumPad5)
                        DrawRGBKeys(g, num6, pen, lbrush, $"6{vbNewLine}→", kbFont, Keys.NumPad6)

                        DrawRGBKeys(g, lshift, pen, lbrush, "Shift", kbFont, Keys.ShiftKey)
                        DrawRGBKeys(g, z, pen, lbrush, "Z", kbFont, Keys.Z)
                        DrawRGBKeys(g, xx, pen, lbrush, "X", kbFont, Keys.X)
                        DrawRGBKeys(g, c, pen, lbrush, "C", kbFont, Keys.C)
                        DrawRGBKeys(g, v, pen, lbrush, "V", kbFont, Keys.V)
                        DrawRGBKeys(g, b, pen, lbrush, "B", kbFont, Keys.B)
                        DrawRGBKeys(g, n, pen, lbrush, "N", kbFont, Keys.N)
                        DrawRGBKeys(g, m, pen, lbrush, "M", kbFont, Keys.M)
                        DrawRGBKeys(g, comma, pen, lbrush, $"<{vbNewLine},", kbFont, Keys.Oemcomma)
                        DrawRGBKeys(g, period, pen, lbrush, $">{vbNewLine}.", kbFont, Keys.OemPeriod)
                        DrawRGBKeys(g, question, pen, lbrush, $"?{vbNewLine}/", kbFont, Keys.OemQuestion)
                        DrawRGBKeys(g, rshift, pen, lbrush, "Shift", kbFont, Keys.ShiftKey)
                        DrawRGBKeys(g, up, pen, lbrush, "↑", kbFont, Keys.Up)
                        DrawRGBKeys(g, num1, pen, lbrush, $"1{vbNewLine}End", kbFont, Keys.NumPad1)
                        DrawRGBKeys(g, num2, pen, lbrush, $"2{vbNewLine}↓", kbFont, Keys.NumPad2)
                        DrawRGBKeys(g, num3, pen, lbrush, $"3{vbNewLine}Pg Dn", kbFont, Keys.NumPad3)
                        DrawRGBKeys(g, numenter, pen, lbrush, $"Enter{vbNewLine}↵", kbFont, Keys.Return)

                        DrawRGBKeys(g, lctrl, pen, lbrush, "Ctrl", kbFont, Keys.ControlKey)
                        DrawRGBKeys(g, lwin, pen, lbrush, "⊞", kbFont, Keys.LWin)
                        DrawRGBKeys(g, lalt, pen, lbrush, "Alt", kbFont, Keys.Menu)
                        DrawRGBKeys(g, space, pen, lbrush, "", kbFont, Keys.Space)
                        DrawRGBKeys(g, ralt, pen, lbrush, "Alt", kbFont, Keys.Menu)
                        DrawRGBKeys(g, rwin, pen, lbrush, "⊞", kbFont, Keys.RWin)
                        DrawRGBKeys(g, menu, pen, lbrush, "▤", kbFont, Keys.Apps)
                        DrawRGBKeys(g, rctrl, pen, lbrush, "Ctrl", kbFont, Keys.ControlKey)
                        DrawRGBKeys(g, left, pen, lbrush, "←", kbFont, Keys.Left)
                        DrawRGBKeys(g, down, pen, lbrush, "↓", kbFont, Keys.Down)
                        DrawRGBKeys(g, right, pen, lbrush, "→", kbFont, Keys.Right)
                        DrawRGBKeys(g, num0, pen, lbrush, $"0{vbNewLine}Ins", kbFont, Keys.NumPad0)
                        DrawRGBKeys(g, numperiod, pen, lbrush, $".{vbNewLine}Del", kbFont, Keys.Decimal)
                    End Using
                End Using
            End Using
        Else
            Using brush As New SolidBrush(KeyboardColor)
                Using pen As New Pen(brush, 2.0F)
                    ...
                    Using kbFont As New Font("Segoe UI", 15.0F, FontStyle.Bold, GraphicsUnit.Pixel)
                        ...
                        DrawLights(g, numlight, pen, My.Computer.Keyboard.NumLock)
                        DrawLights(g, caplight, pen, My.Computer.Keyboard.CapsLock)
                        DrawLights(g, scrlight, pen, My.Computer.Keyboard.ScrollLock)
    ...
    Private Sub DrawRGBKeys(g As Graphics, rect As Rectangle, pen As Pen, lbrush As LinearGradientBrush, text As String, font As Font, key As Keys, Optional tff As TextFormatFlags = TextFormatFlags.Left)
    ...
    Private Sub DrawRGBLights(g As Graphics, rect As Rectangle, pen As Pen, lbrush As LinearGradientBrush, status As Boolean)
        If status Then
    ...
    Private Sub DrawLights(g As Graphics, rect As Rectangle, pen As Pen, status As Boolean)
        Using brush As New SolidBrush(KeyboardColor)
            If status Then
                g.DrawEllipse(pen, rect)
