...
Public Class BaseControl
    ...
    'Keyboard
    Private keyboard As kKeyboard
    Private kbPosX As Integer = (ClientSize.Width / 2) - 800, kbPosY As Integer = (ClientSize.Height / 2) - 225
    Private moveStepX As Integer = 4, moveStepY As Integer = 4
    Private pressingKey As Keys

    Public Property Effect() As eEffect = eEffect.Keyboard
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        ElseIf Effect = eEffect.Keyboard Then
            g.Clear(BackColor)
            keyboard.PaintKeyboard(g, New Point(kbPosX, kbPosY), pressingKey)
        End If
    End Sub
    ...
    Private Sub bgTimer_Tick(sender As Object, e As EventArgs) Handles bgTimer.Tick
        If Effect = eEffect.Keyboard Then MoveKeyboard()

        Invalidate()
    End Sub

    Friend Class kKeyboard

        Public Sub PaintKeyboard(g As Graphics, offset As Point, Optional pressedKey As Keys = Keys.None)
            ...
            Using brush As New SolidBrush(Color.White)
                Using pen As New Pen(brush, 1.5)
                    ...
                    Using kbFont As New Font("Verdana", 15.0F, FontStyle.Bold, GraphicsUnit.Pixel)
                        DrawKeys(g, esc, pen, "ESC", kbFont, Keys.Escape, pressedKey)
                        DrawKeys(g, f1, pen, "F1", kbFont, Keys.F1, pressedKey)
                        DrawKeys(g, f2, pen, "F2", kbFont, Keys.F2, pressedKey)
                        DrawKeys(g, f3, pen, "F3", kbFont, Keys.F3, pressedKey)
                        DrawKeys(g, f4, pen, "F4", kbFont, Keys.F4, pressedKey)
                        DrawKeys(g, f5, pen, "F5", kbFont, Keys.F5, pressedKey)
                        DrawKeys(g, f6, pen, "F6", kbFont, Keys.F6, pressedKey)
                        DrawKeys(g, f7, pen, "F7", kbFont, Keys.F7, pressedKey)
                        DrawKeys(g, f8, pen, "F8", kbFont, Keys.F8, pressedKey)
                        DrawKeys(g, f9, pen, "F9", kbFont, Keys.F9, pressedKey)
                        DrawKeys(g, f10, pen, "F10", kbFont, Keys.F10, pressedKey)
                        DrawKeys(g, f11, pen, "F11", kbFont, Keys.F11, pressedKey)
                        DrawKeys(g, f12, pen, "F12", kbFont, Keys.F12, pressedKey)
                        DrawKeys(g, prnscn, pen, $"Print{vbNewLine}Screen", kbFont, Keys.PrintScreen, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, scroll, pen, $"Scroll{vbNewLine}Lock", kbFont, Keys.Scroll, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, pause, pen, $"Pause{vbNewLine}Break", kbFont, Keys.Pause, pressedKey, TextFormatFlags.HorizontalCenter)

                        DrawKeys(g, tilde, pen, $"~{vbNewLine}`", kbFont, Keys.Oemtilde, pressedKey)
                        DrawKeys(g, d1, pen, $"!{vbNewLine}1", kbFont, Keys.D1, pressedKey)
                        DrawKeys(g, d2, pen, $"@{vbNewLine}2", kbFont, Keys.D2, pressedKey)
                        DrawKeys(g, d3, pen, $"#{vbNewLine}3", kbFont, Keys.D3, pressedKey)
                        DrawKeys(g, d4, pen, $"${vbNewLine}4", kbFont, Keys.D4, pressedKey)
                        DrawKeys(g, d5, pen, $"%{vbNewLine}5", kbFont, Keys.D5, pressedKey)
                        DrawKeys(g, d6, pen, $"^{vbNewLine}6", kbFont, Keys.D6, pressedKey)
                        DrawKeys(g, d7, pen, $"&&{vbNewLine}7", kbFont, Keys.D7, pressedKey)
                        DrawKeys(g, d8, pen, $"*{vbNewLine}8", kbFont, Keys.D8, pressedKey)
                        DrawKeys(g, d9, pen, $"({vbNewLine}9", kbFont, Keys.D9, pressedKey)
                        DrawKeys(g, d0, pen, $"){vbNewLine}0", kbFont, Keys.D0, pressedKey)
                        DrawKeys(g, minus, pen, $"_{vbNewLine}-", kbFont, Keys.OemMinus, pressedKey)
                        DrawKeys(g, plus, pen, $"+{vbNewLine}=", kbFont, Keys.Oemplus, pressedKey)
                        DrawKeys(g, bspace, pen, "Backspace", kbFont, Keys.Back, pressedKey)
                        DrawKeys(g, ins, pen, "Insert", kbFont, Keys.Insert, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, home, pen, $"Home{vbNewLine}⏮", kbFont, Keys.Home, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, pgup, pen, $"Page{vbNewLine}Up{vbNewLine}▲", kbFont, Keys.PageUp, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, numlock, pen, $"Num{vbNewLine}Lock", kbFont, Keys.NumLock, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, numslash, pen, "/", kbFont, Keys.Divide, pressedKey)
                        DrawKeys(g, numstar, pen, "*", kbFont, Keys.Multiply, pressedKey)
                        DrawKeys(g, numminus, pen, "-", kbFont, Keys.Subtract, pressedKey)

                        DrawKeys(g, tab, pen, $"Tab{vbNewLine}⭾", kbFont, Keys.Tab, pressedKey)
                        DrawKeys(g, q, pen, "Q", kbFont, Keys.Q, pressedKey)
                        DrawKeys(g, w, pen, "W", kbFont, Keys.W, pressedKey)
                        DrawKeys(g, e, pen, "E", kbFont, Keys.E, pressedKey)
                        DrawKeys(g, r, pen, "R", kbFont, Keys.R, pressedKey)
                        DrawKeys(g, t, pen, "T", kbFont, Keys.T, pressedKey)
                        DrawKeys(g, yy, pen, "Y", kbFont, Keys.Y, pressedKey)
                        DrawKeys(g, u, pen, "U", kbFont, Keys.U, pressedKey)
                        DrawKeys(g, i, pen, "I", kbFont, Keys.I, pressedKey)
                        DrawKeys(g, o, pen, "O", kbFont, Keys.O, pressedKey)
                        DrawKeys(g, p, pen, "P", kbFont, Keys.P, pressedKey)
                        DrawKeys(g, lbracket, pen, "{" & vbNewLine & "[", kbFont, Keys.Oem4, pressedKey)
                        DrawKeys(g, rbracket, pen, "}" & vbNewLine & "]", kbFont, Keys.Oem6, pressedKey)
                        DrawKeys(g, bslash, pen, $"|{vbNewLine}\", kbFont, Keys.Oem5, pressedKey)
                        DrawKeys(g, del, pen, "Delete", kbFont, Keys.Delete, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, [end], pen, $"End{vbNewLine}⏭", kbFont, Keys.End, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, pgdown, pen, $"Page{vbNewLine}Down{vbNewLine}▼", kbFont, Keys.PageDown, pressedKey, TextFormatFlags.HorizontalCenter)
                        DrawKeys(g, num7, pen, $"7{vbNewLine}Home", kbFont, Keys.NumPad7, pressedKey)
                        DrawKeys(g, num8, pen, $"8{vbNewLine}▲", kbFont, Keys.NumPad8, pressedKey)
                        DrawKeys(g, num9, pen, $"9{vbNewLine}Pg Up", kbFont, Keys.NumPad9, pressedKey)
                        DrawKeys(g, numplus, pen, "+", kbFont, Keys.Add, pressedKey)

                        DrawKeys(g, capslock, pen, $"Caps Lock{vbNewLine}🄰", kbFont, Keys.CapsLock, pressedKey)
                        DrawKeys(g, a, pen, "A", kbFont, Keys.A, pressedKey)
                        DrawKeys(g, s, pen, "S", kbFont, Keys.S, pressedKey)
                        DrawKeys(g, d, pen, "D", kbFont, Keys.D, pressedKey)
                        DrawKeys(g, f, pen, "F", kbFont, Keys.F, pressedKey)
                        DrawKeys(g, gg, pen, "G", kbFont, Keys.G, pressedKey)
                        DrawKeys(g, h, pen, "H", kbFont, Keys.H, pressedKey)
                        DrawKeys(g, j, pen, "J", kbFont, Keys.J, pressedKey)
                        DrawKeys(g, k, pen, "K", kbFont, Keys.K, pressedKey)
                        DrawKeys(g, l, pen, "L", kbFont, Keys.L, pressedKey)
                        DrawKeys(g, semicolon, pen, $":{vbNewLine};", kbFont, Keys.OemSemicolon, pressedKey)
                        DrawKeys(g, quote, pen, $"""{vbNewLine}'", kbFont, Keys.OemQuotes, pressedKey)
                        DrawKeys(g, enter, pen, $"Enter{vbNewLine}⏎", kbFont, Keys.Enter, pressedKey)
                        DrawKeys(g, num4, pen, $"4{vbNewLine}◀", kbFont, Keys.NumPad4, pressedKey)
                        DrawKeys(g, num5, pen, "5", kbFont, Keys.NumPad5, pressedKey)
                        DrawKeys(g, num6, pen, $"6{vbNewLine}▶", kbFont, Keys.NumPad6, pressedKey)

                        DrawKeys(g, lshift, pen, "Shift", kbFont, Keys.ShiftKey, pressedKey)
                        DrawKeys(g, z, pen, "Z", kbFont, Keys.Z, pressedKey)
                        DrawKeys(g, xx, pen, "X", kbFont, Keys.X, pressedKey)
                        DrawKeys(g, c, pen, "C", kbFont, Keys.C, pressedKey)
                        DrawKeys(g, v, pen, "V", kbFont, Keys.V, pressedKey)
                        DrawKeys(g, b, pen, "B", kbFont, Keys.B, pressedKey)
                        DrawKeys(g, n, pen, "N", kbFont, Keys.N, pressedKey)
                        DrawKeys(g, m, pen, "M", kbFont, Keys.M, pressedKey)
                        DrawKeys(g, comma, pen, $"<{vbNewLine},", kbFont, Keys.Oemcomma, pressedKey)
                        DrawKeys(g, period, pen, $">{vbNewLine}.", kbFont, Keys.OemPeriod, pressedKey)
                        DrawKeys(g, question, pen, $"?{vbNewLine}/", kbFont, Keys.OemQuestion, pressedKey)
                        DrawKeys(g, rshift, pen, "Shift", kbFont, Keys.ShiftKey, pressedKey)
                        DrawKeys(g, up, pen, "▲", kbFont, Keys.Up, pressedKey)
                        DrawKeys(g, num1, pen, $"1{vbNewLine}End", kbFont, Keys.NumPad1, pressedKey)
                        DrawKeys(g, num2, pen, $"2{vbNewLine}▼", kbFont, Keys.NumPad2, pressedKey)
                        DrawKeys(g, num3, pen, $"3{vbNewLine}Pg Dn", kbFont, Keys.NumPad3, pressedKey)
                        DrawKeys(g, numenter, pen, $"Enter{vbNewLine}⏎", kbFont, Keys.Return, pressedKey)

                        DrawKeys(g, lctrl, pen, "Ctrl", kbFont, Keys.ControlKey, pressedKey)
                        DrawKeys(g, lwin, pen, "⊞", kbFont, Keys.LWin, pressedKey)
                        DrawKeys(g, lalt, pen, "Alt", kbFont, Keys.Menu, pressedKey)
                        DrawKeys(g, space, pen, "", kbFont, Keys.Space, pressedKey)
                        DrawKeys(g, ralt, pen, "Alt", kbFont, Keys.Menu, pressedKey)
                        DrawKeys(g, rwin, pen, "⊞", kbFont, Keys.RWin, pressedKey)
                        DrawKeys(g, menu, pen, "▤", kbFont, Keys.Apps, pressedKey)
                        DrawKeys(g, rctrl, pen, "Ctrl", kbFont, Keys.ControlKey, pressedKey)
                        DrawKeys(g, left, pen, "◀", kbFont, Keys.Left, pressedKey)
                        DrawKeys(g, down, pen, "▼", kbFont, Keys.Down, pressedKey)
                        DrawKeys(g, right, pen, "▶", kbFont, Keys.Right, pressedKey)
                        DrawKeys(g, num0, pen, $"0{vbNewLine}Ins", kbFont, Keys.NumPad0, pressedKey)
                        DrawKeys(g, numperiod, pen, $".{vbNewLine}Del", kbFont, Keys.Decimal, pressedKey)
        ...
        Private Sub DrawKeys(g As Graphics, rect As Rectangle, pen As Pen, text As String, font As Font, key As Keys, curKey As Keys, Optional tff As TextFormatFlags = TextFormatFlags.Left)
            If curKey = key Then
                g.FillRoundedRectangle(rect, 10, Brushes.White, New RoundedRectCorners(True))
                Dim smaller As New Rectangle(rect.X + 3, rect.Y + 3, rect.Width - 6, rect.Height - 6)
                g.DrawGDIText(text, font, smaller, Color.Black, tff Or TextFormatFlags.Top)
            Else
                g.DrawRoundedRectangle(rect, 10, pen)
                Dim smaller As New Rectangle(rect.X + 3, rect.Y + 3, rect.Width - 6, rect.Height - 6)
                g.DrawGDIText(text, font, smaller, Color.White, tff Or TextFormatFlags.Top)
            End If
        End Sub
